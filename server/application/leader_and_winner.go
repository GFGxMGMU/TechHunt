...
func (app *Application) LeaderBoardAndWinner() error {
	...
	rows, err := app.DB.Pool.Query(context.Background(), "select team_name || ' - ' || name, round_num, entered_at from user_rounds ur natural join users y natural join leader_name leader where ur.submitted=false order by round_num desc, entered_at asc, team_name asc")
	if err != nil {
